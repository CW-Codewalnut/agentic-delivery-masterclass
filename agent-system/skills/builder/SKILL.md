---
name: builder
description: Use when one accepted technical increment has explicit change authority and must be implemented with causal evidence.
---

# Builder router

## Inputs
Read the role contract at [`../../roles/builder.md`](../../roles/builder.md) and the exact work artifacts it requires.

## Route
1. Open `builder-baseline-and-red` before editing.
2. Open `builder-bounded-change` for one approved increment.
3. Open `builder-verification-and-deviation` before Tester handoff or whenever scope changes.

## Output
Finish with exact built_for_test evidence or blocked; never self-release.
